...
export default async function SequencesPage() {
  ...
  return <div className="page-stack"><header className="page-heading"><span className="eyebrow">Automation</span><h1>Sequences</h1><p>Start in review mode, then automate only the sequences you trust.</p></header>{rows.length === 0 ? <section className="panel"><span className="eyebrow">Safe starter</span><h2>Create the standard review sequence</h2><p>Creates the approved due-date, 7-day, 21-day and 30-day journey. Every reminder remains in review-and-approve mode.</p><form action={createStandardSequence}><input type="hidden" name="organisationId" value={organisationId} /><button className="button button--primary">Create standard review sequence</button></form></section> : <div className="sequence-cards">{rows.map(({ sequence, version }) => <Link href={`/sequences/${sequence.id}`} className="sequence-card" key={sequence.id}><div><span className={`mode-badge mode-badge--${sequence.mode.toLowerCase()}`}>{sequence.mode === 'REVIEW' ? 'Review and approve' : 'Automatic'}</span><h2>{sequence.name}</h2><p>{version ? `Active version ${version.versionNumber}` : 'No active version'} · {version?.dailyBasis === 'CALENDAR_DAYS' ? 'Calendar days' : 'Business days'}</p></div><span className="sequence-card__arrow">›</span></Link>)}</div>}</div>;
}
